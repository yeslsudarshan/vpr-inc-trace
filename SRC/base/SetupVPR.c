		...
		OUTP struct s_router_opts *RouterOpts)
{
	...
	{
		if (strcmp(Options.inc_instrument_type, "mid") == 0)
		{
			if (RouterOpts->router_algorithm == READ_ROUTE)
			{
				printf(ERRTAG "Cannot enable --inc_instrument_type midmap with --router_algorithm read_route!\n");
				exit(1);
			}

			RouterOpts->inc_type = INC_MIDMAP;
		}
		else if (strcmp(Options.inc_instrument_type, "post") == 0)
		{
			if (RouterOpts->fixed_channel_width == NO_FIXED_CHANNEL_WIDTH)
			{
				printf(ERRTAG "Please specify --route_chan_width <int> for --inc_instrument_type post!\n");
				exit(1);
			}
			RouterOpts->inc_type = INC_POSTMAP;
		}
		else
		{
	...
	if (RouterOpts->inc_type != INC_NONE && Options.Count[OT_INC_NETS] == NULL)
	{
		printf(ERRTAG "No --inc_nets specified!\n");
		exit(1);
	}
	...
	RouterOpts->inc_many_to_many = TRUE;
	if (Options.Count[OT_INC_MANY_TO_MANY])
	{
		RouterOpts->inc_many_to_many = Options.inc_many_to_many;
	}
