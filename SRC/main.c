...
/*	puts("\t[-packer_algorithm greedy|brute_force]");
    ...
    puts("");
    puts("INCREMENTAL-TRACING options:");
    puts("\t[--inc_instrument_type mid | post ]");
    puts("\t[--inc_nets <comma_separated_string>]");
    puts("\t[--inc_router_algorithm breadth_first | timing_driven | directed_search]");
    puts("\t[--inc_many_to_many on|off] [--inc_le_symmetry on|off]");
    puts("\t[--inc_best_effort on|off] [--inc_dump_all_nets]");
    puts("");
}
